...
`make mujoco-record` produces a bounded four-second APNG and final-contact poster for each variant plus an HTML index and JSON manifest. The camera, sample cadence, safe-stance command, and physics stepping are fixed; feet turn green when the existing public contact interface reports contact, and the enhanced model's passive closure paths are magenta. CI uploads this directory as `mujoco-phase3-review-<commit>` for 14 days. Rasterization can vary across graphics drivers, so the artifact is supplemental review evidence and never replaces deterministic numeric tests.
...
An older, more detailed open-chain export of the mechanism exists in Git history, but it is not part of the runnable baseline and its loop-closing transforms were never completed. The optional enhanced MuJoCo model therefore represents the complete ideal planar topology and lengths from the checked-in kinematics solver with equality constraints; it does not invent the unavailable CAD closure transforms. The simplified URDF remains in place for ROS tooling.
...
3. **Model fidelity** — an optional enhanced model now introduces repository-exported mass/inertia and geometry structure plus the solver-defined ideal planar loop closure, with explicit provenance and approximations. Detailed CAD mechanism recovery, system identification, and hardware validation remain future work.
